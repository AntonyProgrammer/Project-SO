...
void disastrOS_start(void (*f)(void*), void* f_args, char* logfile){
  ...
  Resource_init();
  Descriptor_init();
  //Per utilizzare le funzioni sui semafori
  Semaphore_init();
  SemDescriptor_init();
  init_pcb=0;
  ...
  syscall_numarg[DSOS_CALL_SEMOPEN]      = 2; //NB: 2, non 1
  ...
  syscall_numarg[DSOS_CALL_SEMPOST]      = 1; //NB: 1, non 2
...
int disastrOS_semOpen(int sem_id, int value) {
      return disastrOS_syscall(DSOS_CALL_SEMOPEN, sem_id,value);
}
